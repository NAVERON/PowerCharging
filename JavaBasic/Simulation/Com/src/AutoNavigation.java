...
public class AutoNavigation extends Application {
    ...
    private boolean isPlaying = true, needSave = false;
    ...
    private Parent createContent(){
        ...
        show.setOnMouseReleased(e -> {
            if(e.getButton().equals(MouseButton.PRIMARY)){
                ...
                ship.inShips(ships);
                ship.setDestination(new Point2D(pressedX + ship.getVelocity().getX()*500, pressedY + ship.getVelocity().getY()*500));
    ...
    public void onUpdate(){
        ...
        for(int i = 0; i < ships.size(); i++){   //判断是否碰撞，判断之后是停止运动还是消失？
        ...
        for(int k = 0; k < ships.size(); k++){   //轨迹点的显示，不从history读取，直接每一步绘制
    ...
    public void start(Stage primaryStage) {
        ...
        primaryStage.getScene().setOnKeyPressed(e -> {
            if (!ships.isEmpty()) {
                ...
                }else if(e.getCode() == KeyCode.A){  //测试通信
                    ships.get(0).sendMessage(1, new Message(0, 1, 5.3, new int[]{1, 2}));
                }else if(e.getCode() == KeyCode.B){
                    ships.get(1).sendMessage(0, new Message(1, 0, 3.45, new int[]{0, 2}));
                }
            }
        });
